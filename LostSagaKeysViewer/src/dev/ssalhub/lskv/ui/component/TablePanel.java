 ...
 */
package dev.ssalhub.lskv.ui.component;
...
import dev.ssalhub.lskv.config.group.LocationSettings;
import dev.ssalhub.lskv.config.group.PanelSettings;
import dev.ssalhub.lskv.layout.LayoutValidator;
import dev.ssalhub.lskv.ui.model.EndNumberModel;
import dev.ssalhub.lskv.ui.model.MaxNumberModel;
import dev.ssalhub.lskv.ui.model.SpecialNumberModel.ValueChangeListener;
import dev.ssalhub.lskv.ui.model.SpecialNumberModelEditor;
...
public class TablePanel extends JPanel {
    ...
    /**
     * Checks if this table is operating in live mode where updates are reflected in
     * real time.
     * 
    ...
    public void addPanelRow(SettingList<? extends PanelSettings> panels, PanelSettings info) {
        ...
        if (location) {
            ...
            validator.getXField()
                    .setModel(new EndNumberModel(info.getLayoutX(), validator.getXField(), update(info::setX)));
            validator.getYField()
                    .setModel(new EndNumberModel(info.getLayoutY(), validator.getYField(), update(info::setY)));
            validator.getWidthField().setModel(
                    new MaxNumberModel(info.getLayoutWidth(), validator.getWidthField(), update(info::setWidth)));
            validator.getHeightField().setModel(
                    new MaxNumberModel(info.getLayoutHeight(), validator.getHeightField(), update(info::setHeight)));
