...
public class FileSelector {
    ...
    public static final Path showFileSaveDialog(FileExtension filter, String name) {
        ...
        if (initialised) {
            ...
            while (chooser.showSaveDialog(Dialog.getParentFrame()) == JFileChooser.APPROVE_OPTION) {
                ...
                if (Files.exists(file) && !Dialog
                        .showConfirmDialog(fileName.toString() + " already exists.\nDo you want to replace it?")) {
                    continue;
                }
     ...
     * @throws NullPointerException       When the given description is
     *                                    <code>null</code>.
     ...
     * @throws IllegalStateException      When registering the native file extension
     *                                    failed.
     * @see FileExtension
     */
    public static FileExtension registerFileExtension(String description, String... extensions)
            throws NullPointerException, IllegalArgumentException, TooManyExtensionsException, IllegalStateException {
        ...
        if (System.getProperty("os.name").toLowerCase(Locale.ROOT).contains("windows")) {
            ...
            if (arch.equals("amd64") || arch.equals("x86")) {
                try (InputStream in = ClassLoader
                        .getSystemResourceAsStream("dev/roanh/util/lib/" + arch + "/Util.dll")) {
                    if (in != null) {
                        Path tmp = null;
                        boolean write = false;
                        if (Util.VERSION != null) {
                            tmp = Paths.get(System.getProperty("java.io.tmpdir"))
                                    .resolve("RoanH-Util-v" + Util.VERSION + "-" + arch + ".dll");
